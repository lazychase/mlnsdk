...
namespace mln::net {
	...
	public:
		...
		using fp_default = std::function<void(SessionPtr)>;
		using fp_update = std::function<void(uint64_t)>;
		using fp_noHandler = std::function<void(SessionPtr, ByteStream&)>;
		using fp_initHandler = std::function<void(PacketProcedure*)>;
