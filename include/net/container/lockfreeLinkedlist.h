...
class LockFreeLinkedList {
 ...
 public:
  ...
  bool Find(T& data) {
      ...
      Node* cur;
      HazardPointer prev_hp, cur_hp;
      bool found = SearchReset(data, &prev, &cur, prev_hp, cur_hp);
      return found;
  }
 ...
 private:
  ...
  bool SearchReset(T& data, Node** prev_ptr, Node** cur_ptr,
      HazardPointer& prev_hp, HazardPointer& cur_hp);
...
template <typename T>
bool LockFreeLinkedList<T>::SearchReset(T& data, Node** prev_ptr,
    Node** cur_ptr, HazardPointer& prev_hp,
    HazardPointer& cur_hp) {
    auto& reclaimer = ListReclaimer<T>::GetInstance();
try_again:
