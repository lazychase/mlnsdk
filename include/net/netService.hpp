...
namespace mln::net { 
	...
	static AcceptorPtrType createTcpAcceptor(const size_t acceptorIdx
		, mln::net::ServiceParams& svcParams
	...
	ConnectorPtrType createTcpConnector(const size_t connectorIdx
		, ServiceParams& svcParams
	...
	template <typename EVENT_RECEIVER_TYPE>
	std::shared_ptr<NetService> registConnector(EVENT_RECEIVER_TYPE& eventReceiver
		, boost::asio::io_context& ioc
		, PacketProcedure::CustomPacketParser packetParser
		, PacketManipulator* packetManip
	...
	template <typename EVENT_RECEIVER_TYPE>
	std::shared_ptr<NetService> connect(EVENT_RECEIVER_TYPE& eventReceiver
		, boost::asio::io_context& ioc
		, const std::string ipAddr
		, const uint16_t port
		, const size_t serviceUpdateTimeMs = 1000
